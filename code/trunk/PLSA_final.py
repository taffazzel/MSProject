...
latent_topics=input("Please Enter the number of topics")   #User given
print(latent_topics)
...
total_conference=cnf

number_of_author=len(author)
max_execution=15

kcon_vtopic_prob=defaultdict(list)

for cnfr in inDict.keys():  #Normalizing
    temp = np.random.random(size= (1,latent_topics))
    kcon_vtopic_prob[cnfr].append(temp/np.sum(temp))

ktopic_vauthor_prob=defaultdict(list)

for topic in range(1, latent_topics+1): # Normalizing
    temp = np.random.random(size= (1, len(author)))
    ktopic_vauthor_prob[topic].append(temp/np.sum(temp))
...
for i in range(0,max_execution):
    ...
    for conf in inDict.keys():
        for ath in inDict[conf][0].keys():
            ...
            den=0
            for top in range(1, latent_topics+1):
                den +=kcon_vtopic_prob[conf][0][0][top-1] * ktopic_vauthor_prob[top][0][0][indx]
            for top2 in range(1,latent_topics+1):
                latent_topic_prob[conf][0][ath][0][0][top2-1]= kcon_vtopic_prob[conf][0][0][top2-1] * ktopic_vauthor_prob[top2][0][0][indx]
    ...
    for top in range(1, latent_topics+1):
        ...
        for a in authdictionary.keys():
            ...
            ktopic_vauthor_prob[top][0][0][idx] = det
            temp +=det
        ktopic_vauthor_prob[top][0][0]=(ktopic_vauthor_prob[top][0][0]/temp)
    # maximized P(w|z)
    ...
    for conf in inDict.keys():
        ...
        for topic2 in range(1, latent_topics+1):
            det = 0
            for ath in inDict[conf][0].keys():

                det += inDict[conf][0][ath] * latent_topic_prob[conf][0][ath][0][0][topic2-1]
            kcon_vtopic_prob[conf][0][0][topic2-1] = det
            temp += det
        kcon_vtopic_prob[conf][0][0] = (kcon_vtopic_prob[conf][0][0])/temp
    # maximized P(z|d)
        if 1.0 in kcon_vtopic_prob[conf][0][0]:
            count += 1
    ...
    if count == total_conference: #Coverging point
        print "count = %s " %count
        break;
# E-M finished
...
# sorting
for c in kcon_vtopic_prob.keys():
    temp = (max(kcon_vtopic_prob[c][0][0]))
    temp2=np.where(kcon_vtopic_prob[c][0][0] == temp)
    x=str(temp2)
    y=str(temp)
...
for t in range(1,latent_topics + 1):
    ...
    for c in kcon_vtopic_prob.keys():
        ...
        temp3 = (max(kcon_vtopic_prob[c][0][0]))
        topic_number =(np.where((kcon_vtopic_prob[c][0][0]) == temp3))
